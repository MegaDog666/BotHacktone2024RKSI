...
from get_map.keyboards import start as kbstart
import json
    ...
    async with pool.acquire() as connection:
        ...
        if exists:
            ...
            await message.reply(
                ...
                f"Если хочешь изменить их, используй команду /edit.", reply_markup=kbstart
            )
        else:
...
async def yes(callback: CallbackQuery, state: FSMContext):
    ...
    async with pool.acquire() as connection:
        ...
        if not exists:
            ...
            await callback.message.answer("Спасибо! Твои предпочтения сохранены.", reply_markup=kbstart)
            await state.clear()
        else:
